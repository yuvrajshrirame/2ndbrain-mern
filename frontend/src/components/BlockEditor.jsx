    ...
    if (query.trim().length > 0 && !exactMatch) {
      items.push({
        ...
        onItemClick: async () => {
          ...
          onAddLink(documentId, newNodeId);
          onSyncStatusChange('saved');
        }
      });
    }
...
function BlockEditor({ documentId, onSyncStatusChange, nodes, links, onAddLink, onRemoveLink, user }) {
  ...
  useEffect(() => {
    ...
    // We already have all nodes in memory from Vault's GET /api/graph!
    const node = nodes.find(n => String(n.id) === String(documentId));
    
    if (node && node.content) {
      try {
        setInitialContent(JSON.parse(node.content));
      } catch (err) {
