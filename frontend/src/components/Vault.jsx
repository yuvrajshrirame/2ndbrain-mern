...
function Vault({ user, setUser }) {
  ...
  useEffect(() => {
    ...
    fetch(API_URL + '/api/graph', {
      ...
      .then(data => {
        ...
        if (!selectedNodeId && data.nodes.length > 0) {
          ...
          setLocalTitle(data.nodes[0].name);
        }
      })
    ...
    socket.on('node_updated', (updatedNode) => {
      setGraphData(prev => ({
        ...prev,
    ...
    socket.on('link_added', (link) => {
      setGraphData(prev => ({ ...prev, links: [...prev.links, link] }));
    });
    socket.on('link_deleted', (linkId) => {
      setGraphData(prev => ({
        ...prev,
    ...
    socket.on('folder_added', (folder) => {
      setFolders(prev => [...prev, folder].sort((a, b) => (a.createdAt || 0) - (b.createdAt || 0)));
    });
    socket.on('folder_updated', (updatedFolder) => {
      setFolders(prev => prev.map(f => f.id === updatedFolder.id ? updatedFolder : f));
    });
    socket.on('folder_deleted', (folderId) => {
  ...
  // ==========================================
  // WAITING FOR AUTH (Fallback if popup blocked)
  // ==========================================
  if (!user) {
    return (
      <div style={{ height: '100vh', width: '100vw', background: '#020203', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#fff', fontFamily: 'var(--font-mono)' }}>
         <p style={{ color: '#555', marginBottom: '1.5rem', letterSpacing: '2px', fontSize: '0.85rem' }}>WAITING FOR AUTHENTICATION...</p>
         <button onClick={() => login()} style={{ background: 'transparent', color: 'var(--accent)', border: '1px solid var(--accent)', padding: '0.6rem 1.2rem', fontSize: '0.75rem', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer', borderRadius: '2px', opacity: 0.8 }}>
